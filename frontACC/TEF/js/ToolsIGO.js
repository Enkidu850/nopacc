...
function init2D(){
    ...
    $('<link>')
        ...
        .attr({
            type: 'text/css',
            rel: 'stylesheet',
            href: 'tools/Map2D/leaflet/leaflet.css'
        });
    $.getScript('tools/Map2D/leaflet/leaflet.js', function () {
...
function init2D3D(){
	...
	*/
	...
	$("#btnavigateIGO").append('<div id="btnavigateMode0" class="btnavigateMode ButtonChecked " onclick="navigate.navigateMode(0);" title="Mode de navigation 3D"><img src="tools/MenuDroite/3D.png" class="imgbtnavigatemode" style="height:12px;"/></div>');
	$("#btnavigateIGO").append('<div id="btnavigateMode1" class="btnavigateMode" onclick="navigate.navigateMode(1);" title="Mode de navigation 2D"><img src="tools/MenuDroite/2D.png" class="imgbtnavigatemode"  style="height:12px;"/></div>');
	$("#btnavigateIGO").append('<div id="btnavigateMode2" class="btnavigateMode" onclick="navigate.navigateMode(2);" title="Mode de navigation 2D Nord"><img src="tools/MenuDroite/2DN.png" class="imgbtnavigatemode" style="height:15px;"/></div>');
...
function initSelectLayer(){
	...
	*/
	$("body").append('<div id="divLayers" class="divLayers">' +
		'   <div onclick="setVisibleOL();"><img id="mainImgLayer" class="imgLayer" src="tools/MenuLayer/img/ListeCouche.JPG" alt="Liste des couches"/></div>' +
		...
		'           <img id="imgLayer_Sat" class="imgLayer imgSelected" src="tools/MenuLayer/img/ListeCouche.JPG" alt="Calque principal"/>' +
		'       </div>' +
		'   </div>' +
		'</div>');
...
function selectLayer(id) {
    ...
    if (id !== undefined && id !== SGWorld.ProjectTree.RootID) {
        if (id !== "Sat" && idSel !== id) {
            ...
            $("#mainImgLayer").prop("src", "tools/MenuLayer/img/" + currentName + ".JPG");
            $("#imgLayer_" + id).addClass("imgSelected");

        } else {
            //Force select sur Sat
            $("#mainImgLayer").prop("src", "tools/MenuLayer/img/ListeCouche.JPG");
    ...
    if (idDirLayers !== SGWorld.ProjectTree.RootID) {
        ...
        while (current) {
            ...
            $("#divOthersLayers").append('<div id="divLayer_' + current + '" class="divLayer" onclick="selectLayer(\'' + current + '\');" title="' + currentName + '">' +
                '   <img id="imgLayer_' + current + '" class="imgLayer" src="tools/MenuLayer/img/' + currentName + '.JPG" alt="' + currentName + '"/>' +
                '</div>');
